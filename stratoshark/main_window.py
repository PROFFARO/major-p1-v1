"""
Stratoshark Main Window — Direct Port & Implementation of Stratoshark Desktop Architecture.
...
from stratoshark.core.settings import StratosharkSettings
from stratoshark.core.backend_bridge import BackendBridge
from stratoshark.models.event_table_model import EventTableModel
from stratoshark.models.event_proxy_model import EventFilterProxyModel
from stratoshark.models.detail_tree_model import DetailTreeModel
from stratoshark.widgets.main_toolbar import MainToolBar
from stratoshark.widgets.display_filter_entry import DisplayFilterEntry
from stratoshark.widgets.main_status_bar import MainStatusBar
from stratoshark.widgets.event_byte_view import EventByteView
from stratoshark.widgets.welcome_page import WelcomePage
from stratoshark.widgets.search_frame import StratosharkSearchFrame
from stratoshark.widgets.accordion_frame import GoToFrame
from stratoshark.docks.copilot_dock import CopilotDock
from stratoshark.docks.threat_timeline_dock import ThreatTimelineDock, SubsystemStatusDock
from stratoshark.dialogs.preferences_dialog import PreferencesDialog
from stratoshark.dialogs.coloring_rules_dialog import ColoringRulesDialog
from stratoshark.dialogs.sql_console_dialog import SQLConsoleDialog
from stratoshark.dialogs.io_graph_dialog import StratosharkIOGraphDialog
from stratoshark.dialogs.plot_dialog import StratosharkPlotDialog
from stratoshark.dialogs.capture_options_dialog import CaptureOptionsDialog
from stratoshark.dialogs.threat_forensics_dialog import ThreatForensicsDialog
from stratoshark.dialogs.byte_hasher_dialog import ByteHasherDialog
from stratoshark.resources.icons import StratosharkIcons




class StratosharkMainWindow(QMainWindow):
    """
    Main Window for Stratoshark eBPF & Kernel Threat Analyzer.
    """

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stratoshark · System Call & Kernel Observability Analyzer")
        self.setWindowIcon(StratosharkIcons.get_app_icon())
        self.resize(1340, 860)

        self.settings = StratosharkSettings()
    ...
    def _init_ui(self):
        ...
        self.search_frame = StratosharkSearchFrame(self)
    ...
    def _init_menus(self):
        ...
        add_act(m_help, "&About Stratoshark", self._action_open_about)
    ...
    def _action_open_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Open Stratoshark Capture File",
            "",
            "Stratoshark Telemetry (*.jsonl *.json *.duckdb *.db *.csv);;All Files (*)"
        )
        if not path:
            return
    ...
    def _action_save_file(self):
        path, selected_filter = QFileDialog.getSaveFileName(
            self,
            "Save Stratoshark Capture",
            f"stratoshark_capture_{int(time.time())}.jsonl",
    ...
    def _action_open_io_graphs(self):
        dlg = StratosharkIOGraphDialog(self)
        dlg.exec()

    def _action_open_plot_dialog(self):
        dlg = StratosharkPlotDialog(self)
        dlg.exec()
