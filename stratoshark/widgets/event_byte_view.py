"""
Stratoshark Multi-Format Byte & Hex Dump Viewer.
...
class EventByteView(QWidget):
    """
    Stratoshark multi-format raw byte inspection widget.
    """
    ...
    def _generate_c_array(self, data: bytes) -> str:
        lines = [f"// Stratoshark payload capture ({len(data)} bytes)", "const unsigned char payload[] = {"]
