"""
Stratoshark Welcome Page — System Call & eBPF Kernel Probes Capture Dashboard.
"""
...
from stratoshark.core.theme import ThemeManager, get_ui_font, get_monospace_font
...
class WelcomePage(QWidget):
    """
    Stratoshark Official Welcome Page with full Dark and Light appearance modes.
    """
    ...
    def _init_ui(self):
        ...
        learn_links = [
            ("User's Guide", "stratoshark://guide", "How to capture, filter, and dissect Linux system calls & container activity"),
            ("eBPF Probes Reference", "stratoshark://probes", "Understanding tracepoints, TC ingress/egress, SSL uprobes, and LSM hooks"),
            ("ML Threat Detection Engine", "stratoshark://ml", "Dual XGBoost + PyTorch behavioral model consensus architecture"),
            ("Security Copilot Reference", "stratoshark://copilot", "Interacting with the local LLM Security Analyst in real-time"),
        ]
    ...
    def _populate_extcap_list(self):
        ...
        gear_icon = StratosharkIcons.capture_options()
    ...
    def _on_learn_link_clicked(self, url_str: str):
        guides = {
            "stratoshark://guide": ("Stratoshark User's Guide", "Stratoshark captures system calls and logs from Linux kernel eBPF probes.\n\nDouble-click any probe to stream live events."),
            "stratoshark://probes": ("eBPF Probes Architecture", "• sys_tracer: Monitors sys_enter/sys_exit tracepoints\n• net_filter: Inspects TC socket traffic\n• ssl_tracer: Plaintext TLS payload uprobes\n• lsm_enforcer: Kernel security containment hooks"),
            "stratoshark://ml": ("ML Threat Consensus", "• Random Forest / XGBoost: Rapid anomaly classification\n• PyTorch Deep Neural Network: Sequential behavior modeling\n• Consensus Engine: Zero-false-positive forensic alerts"),
            "stratoshark://copilot": ("Security Copilot Analyst", "Integrated local LLM security analyst ready to triage anomalies and suggest containment commands."),
        }
        title, text = guides.get(url_str, ("Stratoshark Help", "Documentation topic."))
